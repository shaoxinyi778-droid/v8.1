...
export const UploadModal: React.FC<UploadModalProps> = ({ 
    ...
    return new Promise(async (resolve, reject) => {
      ...
      video.onloadedmetadata = async () => {
        try {
          ...
          pointsSet.add(Math.max(0, duration - 0.1));

          let interval = 5;
          if (duration < 5) interval = 1;
          else if (duration <= 20) interval = 2;
          ...
          for (let i = 0; i < uniquePoints.length; i++) {
            ...
            if (foundHuman && thumbnailBase64) break;
          }
  ...
  const processFiles = async () => {
    ...
    for (let i = 0; i < selectedFiles.length; i++) {
      ...
      addLog(`[${i+1}/${totalFiles}] AI 解析中: ${file.name}`, 'loading');
      
      try {
        await new Promise(r => setTimeout(r, 50)); 
        
        // 1. Common Analysis (Same for Guest & Cloud)
        const meta = await processSingleVideo(file);
        const typeStr = meta.orientation === 'portrait' ? '竖屏' : '横屏';
        const humanStr = meta.hasHuman ? '有人像' : '空镜';
        addLog(`解析结果: ${typeStr} / ${humanStr}`, 'ai');
          ...
          <div className="space-y-4">
            ...
            >
              ...
              <p className="text-xs text-gray-400 mt-1">
                  {isGuest ? '本地智能解析 (不消耗流量)' : '本地智能解析 + 云端加密存储'}
              </p>
            </div>
